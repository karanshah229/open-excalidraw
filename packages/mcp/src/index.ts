...
const mcp = new McpServer({ name: 'agentic-whiteboard', version: '0.1.0' })

mcp.registerTool('get_capabilities', {
  ...
  tools: ['get_canvas', 'get_selection', 'get_schema', 'add_elements', 'update_elements', 'delete_elements', 'validate_dag'],
}))

mcp.registerTool('get_canvas', {
...
mcp.registerTool('get_selection', {
  ...
  if (!scene) return failure('whiteboard_offline', { message: 'No browser scene has connected yet.' })
  return json({ revision, selectionIds, elements: scene.elements.filter((element) => selectionIds.includes(String(element.id))) })
})

mcp.registerTool('get_schema', {
  description: 'Return the supported Excalidraw skeleton format for live write operations.',
}, async () => json({
  addElements: { elements: [{ type: 'rectangle', x: 120, y: 120, width: 220, height: 100, label: { text: 'Service' } }] },
  updateElements: { patches: [{ id: 'existing-element-id', changes: { x: 160, y: 120, backgroundColor: '#dbeafe' } }] },
...
mcp.registerTool('add_elements', {
  description: 'Append Excalidraw element skeletons to the live canvas. The browser normalizes skeletons into valid Excalidraw JSON.',
  inputSchema: {
...
mcp.registerTool('update_elements', {
...
mcp.registerTool('delete_elements', {
...
mcp.registerTool('validate_dag', {
  ...
  const visit = (node: string, trail: string[]) => {
    if (visiting.has(node)) { cycles.push([...trail, node]); return }
...
socketServer.on('connection', (socket) => {
  ...
  socket.on('message', (raw) => {
    try {
      const message = JSON.parse(raw.toString()) as { type?: string; scene?: Scene; operationId?: string; selectionIds?: string[] }
    ...
    } catch { socket.send(JSON.stringify({ type: 'error', message: 'Invalid bridge message.' })) }
  })
  socket.on('close', () => adapters.delete(socket))
})

await mcp.connect(new StdioServerTransport())
console.error(`Agentic Whiteboard MCP is ready; browser adapter WebSocket on ws://127.0.0.1:${bridgePort}`)
